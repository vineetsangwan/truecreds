...
const STATUS_COLORS = { new: '#00FF9D', contacted: '#60A5FA', converted: '#34D399', rejected: '#F87171' };
  ...
  return (
    <div data-testid="leads-panel">
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <input className="input-cosmic flex-1 min-w-48" placeholder="Search name, mobile, city..." value={q}
          onChange={e => setQ(e.target.value)} data-testid="leads-search-input" />
        <select className="input-cosmic w-36" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
        ...
        <button className="btn-ghost px-4 py-2.5 text-sm" onClick={load}>↺ Refresh</button>
      </div>
      <div className="mb-4 flex gap-4 text-xs text-slate-500">
        <span>{leads.length} leads found</span>
        <span>·</span>
        <span className="text-[#00FF9D]">{leads.filter(l=>l.status==='new').length} new</span>
        <span className="text-[#60A5FA]">{leads.filter(l=>l.status==='contacted').length} contacted</span>
        <span className="text-[#34D399]">{leads.filter(l=>l.status==='converted').length} converted</span>
      </div>
      {leads.length === 0 ? (
        <div className="text-center text-slate-600 py-16 card-cosmic">No leads found.</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[rgba(255,255,255,0.07)]">
                {['Name','Mobile','City','Loan Amt','Employment','Status','Date','Actions'].map(h => (
                  <th key={h} className="text-left py-3 px-3 text-[10px] text-slate-600 uppercase tracking-wider font-medium whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {leads.map(l => (
                <tr key={l.id} className="border-b border-[rgba(255,255,255,0.04)] hover:bg-[rgba(255,255,255,0.02)] transition-colors" data-testid={`lead-row-${l.id}`}>
                  <td className="py-3 px-3 font-semibold text-sm">{l.full_name}</td>
                  <td className="py-3 px-3 font-mono-nums text-xs text-slate-400">{l.mobile}</td>
                  <td className="py-3 px-3 text-xs text-slate-500">{l.city || '—'}</td>
                  <td className="py-3 px-3 font-mono-nums text-xs text-[#00FF9D] font-bold">{fmtLakh(l.loan_amount)}</td>
                  <td className="py-3 px-3 text-xs text-slate-500">{l.employment_type || '—'}</td>
                  ...
                  <td className="py-3 px-3">
                    <button className="text-red-500 hover:text-red-400 text-xs transition-colors" onClick={() => del(l.id)} data-testid={`lead-delete-${l.id}`}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function WpTab() {
  ...
  const [newPost, setNewPost] = useState({ title: '', content: '', excerpt: '', status: 'publish' });

  useEffect(() => { api.get('/api/wp/config').then(r => setCfg(r.data)); }, []);
  ...
  const test = async () => {
    ...
    try {
      ...
      setLogs(r.data.logs || []);
      toast[r.data.ok ? 'success' : 'error'](r.data.ok ? `Connected! ${r.data.post_count} posts found` : 'Connection failed — check logs');
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-5">
          <div className="card-cosmic">
            <h3 className="font-semibold text-sm mb-5 text-slate-300">WordPress Connection</h3>
            <div className="space-y-4">
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1 block">Site URL</label>
                <input className="input-cosmic" placeholder="https://yourblog.com" value={cfg.base_url}
                  onChange={e => setCfg(p => ({ ...p, base_url: e.target.value }))} data-testid="wp-url-input" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-slate-500 uppercase tracking-wider mb-1 block">WP Username</label>
                  <input className="input-cosmic" placeholder="admin" value={cfg.wp_username || ''}
                    onChange={e => setCfg(p => ({ ...p, wp_username: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs text-slate-500 uppercase tracking-wider mb-1 block">App Password</label>
                  <input className="input-cosmic" type="password" placeholder="xxxx xxxx xxxx xxxx"
                    value={cfg.wp_app_password || ''} onChange={e => setCfg(p => ({ ...p, wp_app_password: e.target.value }))} />
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className={`w-11 h-6 rounded-full cursor-pointer transition-colors relative ${cfg.active ? 'bg-[#00FF9D]' : 'bg-slate-700'}`}
                  onClick={() => setCfg(p => ({ ...p, active: !p.active }))} data-testid="wp-sync-toggle">
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${cfg.active ? 'left-6' : 'left-1'}`} />
                </div>
                <span className="text-sm text-slate-300">Active headless sync <span className="text-slate-600 text-xs">(blog auto-fetches from WP)</span></span>
              </div>
              <div className="flex flex-wrap gap-2">
                <button className="btn-ghost text-xs px-4 py-2" onClick={test} disabled={testing} data-testid="wp-test-btn">
                  {testing ? '⏳ Testing...' : '🔌 Test Connection'}
                </button>
                <button className="btn-mint text-xs px-4 py-2" onClick={save} data-testid="wp-save-btn">💾 Save Config</button>
                {cfg.active && (
                  <button className="btn-ghost text-xs px-4 py-2" onClick={sync} disabled={syncing}>
          ...
          {/* LOGS */}
          {logs.length > 0 && (
            <div className="rounded-xl border border-[rgba(255,255,255,0.07)] overflow-hidden" data-testid="wp-cors-logs">
              <div className="px-4 py-2 bg-[rgba(0,255,157,0.04)] border-b border-[rgba(255,255,255,0.07)]">
                <span className="text-[10px] font-mono text-[#00FF9D] uppercase tracking-wider">Diagnostic Log</span>
              </div>
              <div className="p-4 space-y-1.5" style={{ background: '#050C18', fontFamily: 'JetBrains Mono,monospace' }}>
                {logs.map((l, i) => <div key={i} className="text-xs text-slate-400">{l}</div>)}
              </div>
            </div>
          )}
          ...
          <div className="card-cosmic">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-sm text-slate-300">Publish to WordPress</h3>
              <button className="btn-ghost text-xs px-3 py-1.5" onClick={() => setShowNewPost(!showNewPost)}>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} style={{ overflow: 'hidden' }}>
                  <div className="space-y-3 pt-2">
                    <div>
                      ...
                      <input className="input-cosmic" placeholder="Post title..." value={newPost.title} onChange={e => setNewPost(p => ({ ...p, title: e.target.value }))} />
                    </div>
                    <div>
                      <label className="text-xs text-slate-500 uppercase tracking-wider mb-1 block">Excerpt</label>
                      <input className="input-cosmic" placeholder="Short description..." value={newPost.excerpt} onChange={e => setNewPost(p => ({ ...p, excerpt: e.target.value }))} />
                    </div>
                    <div>
                      <label className="text-xs text-slate-500 uppercase tracking-wider mb-1 block">Content (HTML or Gutenberg blocks)</label>
                      <textarea className="input-cosmic" rows={6} placeholder="<p>Your post content here...</p>" value={newPost.content} onChange={e => setNewPost(p => ({ ...p, content: e.target.value }))} />
                    </div>
                    <div className="flex items-center gap-3">
                      <select className="input-cosmic w-36" value={newPost.status} onChange={e => setNewPost(p => ({ ...p, status: e.target.value }))}>
                        <option value="publish">Publish</option>
                        <option value="draft">Save as Draft</option>
                      </select>
                      <button className="btn-mint text-sm px-5 py-2" onClick={pushPost}>Publish to WP →</button>
                    </div>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            {!showNewPost && <p className="text-xs text-slate-600">Push posts directly to your WordPress site using the REST API with Basic Auth.</p>}
          </div>
        </div>
        ...
        <div className="lg:col-span-5">
          <div className="card-cosmic sticky top-24">
            <h3 className="font-semibold text-sm mb-5 text-slate-300">Setup Guide</h3>
            <ol className="space-y-4">
              {[
                ['Install WordPress', 'Any host works: WP Engine, Kinsta, Cloudways, or even a ₹99/mo shared host.'],
                ['Enable REST API', 'It\'s on by default in WordPress 4.7+. No plugin needed.'],
                ['Add CORS Headers', 'Install "WP CORS" plugin or add `header("Access-Control-Allow-Origin: *")` in functions.php'],
                ['Create App Password', 'WP Admin → Users → Your Profile → Application Passwords. Generate one for "TrueCreds".'],
                ['Paste URL & Credentials', 'Enter your site URL, WP username, and the generated app password above.'],
                ['Test & Save', 'Click Test Connection to verify, then Save. Toggle Active Sync to pull posts automatically.'],
              ].map(([title, desc], i) => (
                <li key={i} className="flex gap-3">
                  <span className="w-6 h-6 rounded-full bg-[rgba(0,255,157,0.1)] border border-[rgba(0,255,157,0.2)] text-[#00FF9D] text-xs font-mono font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{i+1}</span>
                  <div>
...
function StatusTab() {
  ...
  const load = () => api.get('/api/status/system').then(r => setStatus(r.data)).catch(() => {});
  useEffect(() => { load(); const t = setInterval(load, 15000); return () => clearInterval(t); }, []);

  const sendTestEmail = async () => {
    try { const r = await api.post('/api/admin/test-email'); toast[r.data.ok ? 'success' : 'error'](r.data.ok ? 'Test email sent!' : r.data.error); }
    catch { toast.error('Failed to send test email'); }
  };

  if (!status) return <div className="text-slate-600 text-sm py-12 text-center">Loading system status...</div>;

  const Dot = ({ ok }) => (
    <span className={`w-2.5 h-2.5 rounded-full inline-block ${ok ? 'bg-[#00FF9D]' : 'bg-red-500'}`}
      style={ok ? { boxShadow: '0 0 6px rgba(0,255,157,0.6)', animation: 'pulse 2s infinite' } : {}} />
  );

  return (
    <div className="grid sm:grid-cols-2 gap-5" data-testid="status-panel">
      <motion.div className="card-cosmic" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }}>
        <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-sm">Database</h3><Dot ok={status.database.connected} /></div>
        <div className="space-y-1.5 text-xs text-slate-500">
          <div>Name: <span className="text-slate-300 font-mono-nums">{status.database.name}</span></div>
          <div>Status: <span className={status.database.connected ? 'text-[#00FF9D]' : 'text-red-400'}>{status.database.connected ? 'Connected ✓' : 'Disconnected ✗'}</span></div>
        </div>
      </motion.div>

      <motion.div className="card-cosmic" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
        <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-sm">WordPress Sync</h3><Dot ok={status.wordpress.active} /></div>
        <div className="space-y-1.5 text-xs text-slate-500">
          <div>Active: <span className={status.wordpress.active ? 'text-[#00FF9D]' : 'text-slate-400'}>{status.wordpress.active ? 'Yes' : 'No'}</span></div>
          <div>Posts cached: <span className="text-slate-300 font-mono-nums">{status.wordpress.post_count}</span></div>
          <div>Auth: <span className="text-slate-300">{status.wordpress.has_auth ? '✓ App Password set' : '— Not configured'}</span></div>
          {status.wordpress.last_tested && <div>Last test: <span className="text-slate-400">{new Date(status.wordpress.last_tested).toLocaleString('en-IN')}</span></div>}
        </div>
      </motion.div>

      <motion.div className="card-cosmic" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }}>
        <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-sm">Email (Resend)</h3><Dot ok={status.email.configured} /></div>
        <div className="space-y-1.5 text-xs text-slate-500 mb-4">
          <div>Configured: <span className={status.email.configured ? 'text-[#00FF9D]' : 'text-slate-400'}>{status.email.configured ? 'Yes ✓' : 'No — Add RESEND_API_KEY'}</span></div>
          {status.email.from && <div>From: <span className="text-slate-300 font-mono-nums">{status.email.from}</span></div>}
          {status.email.to && <div>Notify: <span className="text-slate-300 font-mono-nums">{status.email.to}</span></div>}
        </div>
        <button className="btn-ghost text-xs px-3 py-2" onClick={sendTestEmail} data-testid="send-test-email-btn">Send Test Email</button>
      </motion.div>

      <motion.div className="card-cosmic" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 }}>
        <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-sm">Counters</h3><Dot ok={true} /></div>
        <div className="grid grid-cols-2 gap-3">
          {[['Leads', status.counters.total_leads], ['Loan Apps', status.counters.loan_apps], ['Blog Posts', status.counters.blog_posts], ['Contacts', status.counters.contacts]].map(([l, v]) => (
            <div key={l} className="text-center p-2 rounded-lg bg-[rgba(255,255,255,0.03)]">
              <div className="font-mono-nums text-lg text-white font-bold">{v}</div>
              <div className="text-[9px] text-slate-600 uppercase tracking-wider">{l}</div>
            </div>
          ))}
        </div>
        <div className="mt-3 text-center text-[10px] text-slate-600 font-mono">v{status.counters.version} · {status.counters.service}</div>
      </motion.div>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const logout = () => { localStorage.removeItem('truecreds_token'); nav('/admin/login'); };

  useEffect(() => {
    api.get('/api/auth/me').catch(() => { localStorage.removeItem('truecreds_token'); nav('/admin/login'); });
  }, [nav]);
  ...
  return (
    <div className="min-h-screen" style={{ background: '#070F1E' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" data-testid="admin-dashboard-page">
        <div className="mb-8">
          <h1 className="font-black text-3xl" style={{ fontFamily: 'Outfit,sans-serif' }}>Command Center</h1>
          <p className="text-slate-500 text-sm mt-1">Manage leads, WordPress sync, and system health.</p>
        </div>
